...
from torch.nn import Module, Sequential, LeakyReLU, Parameter
from torch.nn import functional as F
from torch.nn.init import ones_, zeros_

from gans.gan_module import GanModule
from gans.karras_2017_pggan import EqualizedLinear, EqualizedConv2d, PgGanDiscriminator, PgGanDiscriminatorTransition
...
class MappingModule(GanModule):
    def __init__(self,
        ...
        self.sequential = Sequential(
            EqualizedLinear(in_features=latent_vector_size, out_features=latent_vector_size),
...
class GeneratorBlock(GanModule):
    def __init__(self,
        ...
        self.weight_to_new_mean_1 = EqualizedLinear(in_features=latent_vector_size, out_features=out_channels)
        self.weight_to_new_std_1 = EqualizedLinear(in_features=latent_vector_size, out_features=out_channels)
        ...
        self.weight_to_new_mean_2 = EqualizedLinear(in_features=latent_vector_size, out_features=out_channels)
        self.weight_to_new_std_2 = EqualizedLinear(in_features=latent_vector_size, out_features=out_channels)
...
def to_rgb_layer(size: int, num_channels_by_image_size: Dict[int, int] = None):
    if num_channels_by_image_size is None:
        num_channels_by_image_size = NUM_CHANNELS_BY_IMAGE_SIZE
    return EqualizedConv2d(in_channels=num_channels_by_image_size[size],
                           out_channels=3,
                           kernel_size=1,
                           stride=1,
                           padding=0)
...
class Karras2018StyleGan(StyleGan):
    def __init__(self,
