...
class FirstNOutputLsGanForDiscriminatorLoss(PoserGanLoss):
    ...
    def compute_with_cached_outputs(self, G: PoserGanModule, D: PoserGanModule,
        ...
        real_score_loss = image_average((D(source_image, pose, real_target_image)[0] - 1) ** 2).mean()
        ...
        for i in range(len(self.weights)):
            ...
            if weight > 0:
                ...
                fake_score = D(source_image, pose, fake_target_image)[0]
                lsgan_loss = real_score_loss + image_average(fake_score ** 2).mean()
                image_loss = weight * lsgan_loss
            else:
