...
class PoserGanRotationOnlyFromThreeStepDataDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        posed_image = self.load_image(example[4])
        if posed_image.shape[1] != self.image_size or posed_image.shape[2] != self.image_size:
            raise RuntimeError(example[4])
        _pose = example[3][:self.bone_parameters_count]
        pose = torch.Tensor(_pose)
        return [morphed_image, pose, posed_image]
...
if __name__ == "__main__":
    dataset = PoserGanRotationOnlyFromThreeStepDataDataset(
        "data/rindou/_20190906/three_step_varying_ambience/train/data.tsv",
        pose_size=6,
